...
function registerDotsBoxesHandlers(io, socket) {
  ...
  socket.on("disconnect", () => {
    try {
      ...
      console.log(
        `[DB Disconnect] ${player.playerName} left room ${room.roomCode}`
      );

      io.to(room.roomCode).emit("db_player_disconnected", {
        playerName: player.playerName,
        message: `${player.playerName} disconnected. Waiting for reconnect...`,
      });

      setTimeout(() => {
        const currentRoom = DotsBoxesStore.get(room.roomCode);
        if (currentRoom) {
          ...
          if (stillDisconnected) {
            io.to(room.roomCode).emit("db_room_closed", {
              message: `${player.playerName} left. Room has been closed.`,
            });
            DotsBoxesStore.delete(room.roomCode);
            console.log(`[DB Room Closed] ${room.roomCode}`);
          }
        }
      }, 30000);
    } catch (err) {
