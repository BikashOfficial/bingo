...
export function useSocketHandlers() {
  ...
  useEffect(() => {
    ...
    socket.on('line_completed', ({ completedLines, newLines }) => {
      ...
      toast(`Line ${totalNow} complete! ${totalNow < 5 ? letters[totalNow - 1] + ' ✓' : 'BINGO!'}`, {
        icon: totalNow === 5 ? '🎉' : '⚡',
    ...
    return () => {
      ...
      const events = [
        ...
        'line_completed', 'bingo_won', 'chat_message', 'player_disconnected',
        'player_reconnected', 'game_reset', 'player_wants_rematch', 'room_closed',
        'not_your_turn',
      ];
